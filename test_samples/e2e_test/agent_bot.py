...
client = AsyncAzureOpenAI(
    ...
    api_key=environ.get("AZURE_OPENAI_ENDPOINT"),
)
...
async def on_poem_message(context: TurnContext, state: TurnState):
    ...
    try:
        await context.streaming_response.queue_informative_update(
        ...
        stream = await client.responses.create(
            ...
            """,
            input="Write a poem about the Greek God Apollo as depicted in the Percy Jackson books",
            stream=True,
            max_output_tokens=1000
        )

        async for update in stream:
            delta = update.choices[0].delta.get("content")
            if delta:
