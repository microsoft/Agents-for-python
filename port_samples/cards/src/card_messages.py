...
class CardMessages:
    ...
    async def send_intro_card(context: TurnContext):
        ...
        card = CardFactory.hero_card(HeroCard(title="Cards", buttons=buttons, text="Select one of the following choices"))
    ...
    async def send_audio_card(context: TurnContext):
        card = CardFactory.audio_card(AudioCard(
            title="I am your father",
            media=[MediaUrl(url="https://www.mediacollege.com/downloads/sound-effects/star-wars/darthvader/darthvader_yourfather.wav", profile="Darth Vader - I am your father")],
            buttons=[CardAction(type=ActionTypes.open_url, title="Read more", value="https://en.wikipedia.org/wiki/The_Empire_Strikes_Back")],
            subtitle="Star Wars: Episode V - The Empire Strikes Back",
            text="The Empire Strikes Back (also known as Star Wars: Episode V - The Empire Strikes Back) is a 1980 American epic space opera film directed by Irvin Kershner. Leigh Brackett and Lawrence Kasdan wrote the screenplay, with George Lucas writing the film's story and serving as executive producer. The second installment in the original Star Wars trilogy, it was produced by Gary Kurtz for Lucasfilm Ltd. and stars Mark Hamill, Harrison Ford, Carrie Fisher, Billy Dee Williams, Anthony Daniels, David Prowse, Kenny Baker, Peter Mayhew and Frank Oz.",
            image=ThumbnailUrl(url="https://upload.wikimedia.org/wikipedia/en/3/3c/SW_-_Empire_Strikes_Back.jpg", alt="The Empire Strikes Back"),
            aspect="16:9",
            duration="PT2M"
        ))
        await CardMessages.send_activity(context, card)
    ...
    async def send_hero_card(context):
        card = CardFactory.hero_card(HeroCard(
            ...
            images=[CardImage(url="https://blogs.microsoft.com/wp-content/uploads/prod/2023/09/Press-Image_FINAL_16x9-4.jpg")],
            buttons=[CardAction(type=ActionTypes.open_url, title="Get started", value="https://docs.microsoft.com/en-us/azure/bot-service/")],
        ))
        await CardMessages.send_activity(context, card)

    @staticmethod
    async def send_receipt_card(context):
        card = CardFactory.receipt_card(ReceiptCard(
            title="John Doe",
            ...
            total="$90.95",
            buttons=[
                CardAction(type=ActionTypes.open_url, title="More information", value="https://azure.microsoft.com/en-us/pricing/details/bot-service/"),
            ]
        ))
        await CardMessages.send_activity(context, card)
    ...
    async def send_thumbnail_card(context):
        card = CardFactory.thumbnail_card(ThumbnailCard(
