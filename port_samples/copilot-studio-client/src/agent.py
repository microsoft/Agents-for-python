...
def acquire_token(settings: ConnectionSettings, app_client_id, tenant_id):
    ...
    try:
        ...
        retry_interactive = True
        logger.error(f"Error acquiring token silently: {e}. Going to attempt interactive login.")
...
def create_client():
    ...
    token = acquire_token(settings, app_client_id=environ.get("app_client_id"),
            ...
            if reply.type == ActivityTypes.message:
                ...
                if reply.suggested_actions:
                    for action in reply.suggested_actions.actions:
                        print(f" - {action.title} ({action.type})")
