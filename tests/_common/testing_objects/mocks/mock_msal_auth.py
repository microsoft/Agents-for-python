...
class MockMsalAuth(MsalAuth):
    ...
    def _create_client_application(self) -> None:
        self._msal_auth_client = self.mock_client
...
def agentic_mock_class_MsalAuth(
    ...
    mocker.patch.object(
        ...
        return_value=get_agentic_application_token_return,
    )
    mocker.patch.object(
        MsalAuth,
        "get_agentic_instance_token",
        return_value=get_agentic_instance_token_return,
    )
    mocker.patch.object(
        MsalAuth, "get_agentic_user_token", return_value=get_agentic_user_token_return
    )
