...
def _create_turn_context(
    ...
    *,
    ...
    context.send_activity = mocker.AsyncMock(side_effect=send_side_effect)
    return context
...
async def test_queue_text_chunk_and_end_stream_send_streaming_then_final_message(mocker):
    context = _create_turn_context(
        ...
        send_side_effect=[ResourceResponse(id="stream-1")],
    )
    response = StreamingResponse(context)
...
async def test_set_citations_only_sends_final_when_end_stream_happens_before_drain(mocker):
    context = _create_turn_context(
        mocker,
        delivery_mode=DeliveryModes.stream,
        send_side_effect=[ResourceResponse(id="stream-2")],
    )
    response = StreamingResponse(context)
...
async def test_set_citations_adds_only_used_citations_when_streaming_activity_is_sent(mocker):
    context = _create_turn_context(
        mocker,
        delivery_mode=DeliveryModes.stream,
        send_side_effect=[ResourceResponse(id="stream-2"), ResourceResponse(id="stream-2")],
    )
    response = StreamingResponse(context)
    response.set_citations(
        [
...
async def test_end_stream_cannot_be_called_twice(mocker):
    context = _create_turn_context(
        mocker,
        delivery_mode=DeliveryModes.stream,
        send_side_effect=[ResourceResponse(id="stream-3")],
    )
    response = StreamingResponse(context)
...
async def test_teams_403_marks_stream_as_cancelled_and_future_chunks_are_ignored(mocker):
    context = _create_turn_context(
        ...
        send_side_effect=Exception("403 forbidden"),
    )
    response = StreamingResponse(context)
...
async def test_feedback_loop_type_added_to_final_streaminfo_entity(mocker):
    context = _create_turn_context(
        mocker,
        delivery_mode=DeliveryModes.stream,
        send_side_effect=[ResourceResponse(id="stream-4")],
    )
    response = StreamingResponse(context)
...
async def test_generated_by_ai_label_adds_ai_entity_on_final_message(mocker):
    context = _create_turn_context(
        mocker,
        delivery_mode=DeliveryModes.stream,
        send_side_effect=[ResourceResponse(id="stream-5")],
    )
    response = StreamingResponse(context)
