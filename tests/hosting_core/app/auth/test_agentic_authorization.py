...
#         return AgenticAuthorization(storage, connection_manager, **AGENTIC_ENV_DICT)
...
#         assert activity.is_agentic() == AgenticAuthorization.is_agentic_request(
#             activity
#         )
...
#         context = self.TurnContext(mocker, activity=activity)
#         assert (
#             AgenticAuthorization.get_agent_instance_id(context)
...
#         context = self.TurnContext(mocker, activity=activity)
#         assert (
#             AgenticAuthorization.get_agentic_user(context) == DEFAULTS.agentic_user_id
#         )
...
#             return_value=[DEFAULTS.token, "bot_id"]
#         )
...
#             MemoryStorage(), connection_manager, **AGENTIC_ENV_DICT
#         )
...
#     @pytest.mark.asyncio
#     async def test_sign_in_success(self, mocker, agentic_auth):
#         mocker.patch.object(
#             AgenticAuthorization, "get_agentic_user_token", return_value=DEFAULTS.token
#         )
#         res = await agentic_auth.sign_in(None, ["user.Read"])
#         assert res.token_response.token == DEFAULTS.token
#         assert res.tag == FlowStateTag.COMPLETE

#     @pytest.mark.asyncio
#     async def test_sign_in_failure(self, mocker, agentic_auth):
#         mocker.patch.object(
#             AgenticAuthorization, "get_agentic_user_token", return_value=None
#         )
#         res = await agentic_auth.sign_in(None, ["user.Read"])
