...
class AgenticMsalAuth(MsalAuth):
    ...
    # to avoid this
    async def get_agentic_application_token(self, agent_app_instance_id: str) -> Optional[str]:
    ...
    async def get_agentic_instance_token(self, agent_app_instance_id: str) -> Optional[str]:

        if not agent_app_instance_id:
            raise ValueError("Agent application instance ID must be provided.")
        
        agent_token_result = self.get_agentic_application_token(agent_app_instance_id)
        ...
        if agent_instance_token:
            return agent_instance_token.get("access_token")
