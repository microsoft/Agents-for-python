...
class TestFlowStorageClient:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "auth_handler_id", ["handler", "auth_handler"]
    )
    ...
    async def test_integration_with_memory_storage(self, channel_id, user_id):
        ...
        await baseline.write({f"auth/{channel_id}/{user_id}/handler": new_flow_state_alpha.model_copy()})
        await baseline.write({f"auth/{channel_id}/{user_id}/chi": flow_state_chi.model_copy()})

        await write_both({f"auth/{channel_id}/{user_id}/handler": new_flow_state_alpha.model_copy()})
        await write_both({f"auth/{channel_id}/{user_id}/auth_handler": flow_state_beta.model_copy()})
        ...
        await read_check([f"auth/{channel_id}/{user_id}/auth_handler"], target_cls=FlowState)
