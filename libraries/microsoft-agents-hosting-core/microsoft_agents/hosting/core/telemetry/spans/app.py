...
class AppOnTurn(SimpleSpanWrapper):
    ...
    def __init__(self, turn_context: TurnContextProtocol):
        ...
        """
        super().__init__(constants.SPAN_APP_ON_TURN)
        ...
        if error is None:
            ...
            _metrics.turn_duration.record(
                duration,
                {
                    ...
                    constants.ATTR_ACTIVITY_CHANNEL_ID: self._turn_context.activity.channel_id or constants.UNKNOWN,
                },
            )
        else:
    ...
    def _get_attributes(self) -> AttributeMap:
        return {
            constants.ATTR_CONVERSATION_ID: get_conversation_id(self._turn_context.activity),
            constants.ATTR_ACTIVITY_CHANNEL_ID: self._turn_context.activity.channel_id or constants.UNKNOWN,
            constants.ATTR_SERVICE_URL: self._turn_context.activity.service_url,
        }
    ...
    def share(self, route_authorized: bool, route_matched: bool) -> None:
        ...
        if self._span is not None:
            self._span.set_attribute("app_run.route.authorized", route_authorized)
            self._span.set_attribute("app_run.route.matched", route_matched)
...
class AppRouteHandler(SimpleSpanWrapper):
    ...
    def __init__(self, turn_context: TurnContextProtocol):
        ...
        super().__init__(constants.SPAN_APP_ROUTE_HANDLER)
        self._turn_context = turn_context
...
class AppBeforeTurn(SimpleSpanWrapper):
    ...
    def __init__(self):
        ...
        super().__init__(constants.SPAN_APP_BEFORE_TURN)
...
class AppAfterTurn(SimpleSpanWrapper):
    ...
    def __init__(self):
        ...
        super().__init__(constants.SPAN_APP_AFTER_TURN)
...
class AppDownloadFiles(SimpleSpanWrapper):
    ...
    def __init__(self, turn_context: TurnContextProtocol):
        ...
        super().__init__(constants.SPAN_APP_DOWNLOAD_FILES)
        self._turn_context = turn_context

    def _get_attributes(self) -> AttributeMap:
        return {
            constants.ATTR_ATTACHMENT_COUNT: len(self._turn_context.activity.attachments or []),
        }