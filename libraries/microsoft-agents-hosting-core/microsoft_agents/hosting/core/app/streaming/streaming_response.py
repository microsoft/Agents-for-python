...
class StreamingResponse:
    ...
    def _add_feedback_loop_to_channel_data(self, activity: Activity) -> None:
        channel_data = activity.channel_data
        if channel_data is None:
            serialized_channel_data = {}
        elif isinstance(channel_data, Mapping):
            serialized_channel_data = dict(channel_data)
        elif callable(model_dump := getattr(channel_data, "model_dump", None)):
            serialized_channel_data = model_dump(
                mode="json",
                by_alias=True,
                exclude_none=True,
                exclude_unset=True,
            ...
            logger.warning(
                "Unable to add feedback loop to unsupported channel data type: %s",
                type(channel_data).__name__,
            )
            return

        serialized_channel_data["feedbackLoop"] = {
