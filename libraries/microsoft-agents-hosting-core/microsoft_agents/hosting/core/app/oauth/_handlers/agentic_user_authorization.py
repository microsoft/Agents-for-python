...
class AgenticUserAuthorization(_AuthorizationHandler):
    ...
    async def get_agentic_instance_token(self, context: TurnContext) -> TokenResponse:
        ...
        if not tenant_id:
            logger.error(
                "Unable to retrieve agentic instance token: missing agentic tenant Id. Agentic Tenant ID: %s",
                tenant_id,
            )
            raise ValueError(
                f"Unable to retrieve agentic instance token: missing agentic tenant Id. Agentic Tenant ID: {tenant_id}"
            )
    ...
    async def get_agentic_user_token(
        ...
        if not tenant_id:
            logger.error(
                "Unable to retrieve agentic user token: missing agentic tenant Id. Agentic Tenant ID: %s",
                tenant_id,
            )
            raise ValueError(
                f"Unable to retrieve agentic user token: missing agentic tenant Id. Agentic Tenant ID: {tenant_id}"
            )
