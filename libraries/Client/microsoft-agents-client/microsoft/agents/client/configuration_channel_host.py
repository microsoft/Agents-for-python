...
class ConfigurationChannelHost(ChannelHostProtocol):
    def __init__(self, channel_factory: ChannelFactoryProtocol, connections: Connections, configuration: ChannelsConfiguration, default_channel_name: str):
    ...
    def get_channel_from_channel_info(self, channel_info: ChannelInfoProtocol) -> ChannelProtocol:
        if not channel_info:
            raise ValueError(f"ConfigurationChannelHost.get_channel_from_channel_info(): channel_info cannot be None")
        
        token_provider = self.connections.get_token_provider(channel_info.token_provider)
        if not token_provider:
            raise ValueError(f"ConfigurationChannelHost.get_channel_from_channel_info(): token_provider not found for '{channel_info.token_provider}'")
