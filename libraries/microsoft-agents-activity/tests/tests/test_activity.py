...
class TestActivityConversationOps:
    ...
    def create_activity(self, locale):
        pass

    @pytest.fixture
    def activity(self):
        return self.create_activity("en-us")

    def conversation_assert_helper(self, activity, conversation_reference):
        assert activity.id == conversation_reference.activity_id
        assert activity.from_property.id == conversation_reference.from_property.id
        assert activity.recipient.id == conversation_reference.bot.id
        assert activity.conversation.id == conversation_reference.conversation.id
        assert activity.channel.id == conversation_reference.channel.id
    ...
    def test_apply_conversation_reference_is_incoming(self):
        activity = self.create_activity("en-uS") # on purpose
        conversation_reference = ConversationReference(
            ...
            locale = "en-us",
            delivery_mode = DeliveryModes.ExpectReplies
        )
    ...
    @pytest.mark.parametrize("locale", [None, "en-uS"])
    def test_apply_conversation_reference(self, locale):
        activity = self.create_activity(locale)
        conversation_reference = ConversationReference(
            channel_id = "123",
    ...
    def test_create_trace(self, value, value_type, create_recipient, create_from, label):
        activity = self.create_activity("en-us", create_recipient, create_from)
    ...
    def test_can_create_reply_activity(self, activity_locale, text, create_recipient, create_from, create_reply_locale):
        activity = self.create_activity(activity_locale, create_recipient, create_from)
