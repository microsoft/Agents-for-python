...
def test_sanitize_key_suffix_with_truncation():
    ...
    assert result.endswith(suffix) or len(result) == 255  # Either has suffix or was truncated
...
def test_sanitize_key_integration():
    ...
    result_no_trunc = sanitize_key(base_key, key_suffix=suffix, compatibility_mode=False)
    assert "*47" in result_no_trunc or "*92" in result_no_trunc  # Contains sanitized chars
