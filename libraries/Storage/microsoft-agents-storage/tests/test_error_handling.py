...
async def test_is_status_code_with_status_code_check():
    ...
    assert await ignore_error(raise_custom_error(403), is_status_code_error(403)) is None

    with pytest.raises(CustomError) as err:
        assert await ignore_error(raise_custom_error(404), is_status_code_error(500)) is None
