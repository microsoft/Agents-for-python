...
```bash
pip install -e ./microsoft-agents-hosting-testing/ --config-settings editable_mode=compat
```
...
```python
import pytest
from microsoft_agents.hosting.testing import AiohttpScenario, AgentEnvironment
...
```python
from microsoft_agents.hosting.testing import ExternalScenario
...
```python
from microsoft_agents.hosting.testing.utils import contains
...
```python
from microsoft_agents.hosting.testing import Select
...
```python
from microsoft_agents.hosting.testing import ConversationTranscriptFormatter
...
Run a sample by file name:

```powershell
python .\docs\samples\{SAMPLE_NAME}.py
```
...
python .\docs\samples\utilities.py
```
