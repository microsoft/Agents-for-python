...
from microsoft_agents.hosting.testing.core.agent_client import AgentClient
from microsoft_agents.hosting.testing.core._aiohttp_client_factory import _AiohttpClientFactory
from microsoft_agents.hosting.testing.core.config import ClientConfig
from microsoft_agents.hosting.testing.core.external_scenario import ExternalScenario
from microsoft_agents.hosting.testing.core.scenario import ScenarioConfig
from microsoft_agents.hosting.testing.core.fluent import (
    ActivityTemplate,
    Expect,
    Select,
)
from microsoft_agents.hosting.testing.core.transport import (
    Transcript,
    Exchange,
    AiohttpSender,
    AiohttpCallbackServer,
...
class TestAgentClientWithAiohttpSender:
    ...
    async def test_client_full_conversation_flow(self):
        ...
        mock_server.on_text(
            "hello", Activity(type=ActivityTypes.message, text="Hi there!")
        )
        mock_server.on_text(
            "bye", Activity(type=ActivityTypes.message, text="Goodbye!")
        )
        mock_server.default_response(
            Activity(type=ActivityTypes.message, text="I don't understand")
        )
...
class TestAiohttpCallbackServerIntegration:
    ...
    async def test_callback_server_multiple_activities(self):
        ...
        async with callback_server.listen() as transcript:
            async with ClientSession() as session:
                for i in range(3):
                    activity = Activity(
                        type=ActivityTypes.message, text=f"Message {i+1}"
                    )
                    await session.post(
    ...
    async def test_callback_server_shares_transcript(self):
        ...
        parent_transcript.record(
            Exchange(request=Activity(type=ActivityTypes.message, text="Initial"))
        )
...
class Test_AiohttpClientFactoryIntegration:
    ...
    async def test_factory_creates_working_client(self):
        ...
        mock_server.default_response(
            Activity(type=ActivityTypes.message, text="Factory test OK")
        )
        agent_endpoint = f"{mock_server.endpoint}/api/messages"
    ...
    async def test_factory_creates_multiple_clients(self):
        ...
        async with mock_server.run():
            ...
            try:
                client1 = await factory(ClientConfig())
                client2 = await factory(ClientConfig())
...
class TestExternalScenarioIntegration:
    ...
    def test_external_scenario_accepts_custom_config(self):
        ...
        scenario = ExternalScenario(
            endpoint="http://localhost:3978", config=custom_config
        )
...
class TestEndToEndIntegration:
    ...
    async def test_multi_user_http_conversation(self):
        ...
        mock_server.default_response(
            Activity(type=ActivityTypes.message, text="Received")
        )
        agent_endpoint = f"{mock_server.endpoint}/api/messages"
        ...
        async with mock_server.run():
            ...
            factory = _AiohttpClientFactory(
                ...
                sdk_config={},
                default_template=ActivityTemplate(
                    **{"conversation.id": "multi-user-conv"}
                ),
                default_config=ClientConfig(),
                transcript=transcript,
            )
            ...
            try:
                ...
                bob = await factory(
                    ClientConfig(activity_template=ActivityTemplate({"from.id": "bob"}))
                )
    ...
    async def test_invoke_and_message_mixed_flow(self):
        ...
        mock_server.default_response(
            Activity(type=ActivityTypes.message, text="Message received")
        )
        ...
        async with mock_server.run():
            async with ClientSession() as session:
                ...
                invoke_exchanges = [
                    ex for ex in history if ex.request.type == ActivityTypes.invoke
                ]
    ...
    async def test_select_and_expect_with_http_responses(self):
        ...
        async with mock_server.run():
            async with ClientSession() as session:
                ...
                messages = (
                    Select(responses)
                    .where(lambda x: x.type == ActivityTypes.message)
                    .get()
                )
                ...
                typing = (
                    Select(responses)
                    .where(lambda x: x.type == ActivityTypes.typing)
                    .get()
                )
                assert len(typing) == 1
                ...
                events = (
                    Select(responses)
                    .where(lambda x: x.type == ActivityTypes.event)
                    .get()
                )
                assert len(events) == 1
