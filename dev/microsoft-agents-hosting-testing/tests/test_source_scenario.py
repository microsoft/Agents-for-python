...
from microsoft_agents.hosting.testing import SourceScenario
import microsoft_agents.hosting.testing.constants as constants
...
class TestRunScriptPwshDetection:
    ...
    async def test_prefers_pwsh_over_powershell(self, tmp_path):
        ...
        with patch("shutil.which", side_effect=_fake_which), patch(
            ...
            "microsoft_agents.hosting.testing.source_scenario.asyncio.sleep",
            new_callable=AsyncMock,
        ), patch(
            "microsoft_agents.hosting.testing.source_scenario._terminate_tree"
        ):
            async with scenario._run_script():
...
class TestSourceScenarioLifecycle:
    ...
    async def test_two_scenarios_sequentially_stop_on_context_exit(self, tmp_path):
        ...
        with patch(
            "microsoft_agents.hosting.testing.source_scenario.subprocess.Popen",
