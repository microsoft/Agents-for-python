...
from microsoft_agents.hosting.testing.cli.core import (
    async_command,
    pass_output,
    Output,
    with_scenario,
)
from microsoft_agents.hosting.testing.core import Exchange, Scenario
...
async def run_load_test(
    scenario: Scenario, activity: Activity, num: int, timeout: float
) -> list[RunResult | None]:
    ...
    async with scenario.run() as client_factory:
        ...
        async def send_activity(run_id: int):
            ...
            try:
                ...
                results[run_id] = RunResult(
                    latency=None, error=True, error_message=str(e)
                )

        await asyncio.gather(*[send_activity(i) for i in range(num)])
...
def show_results(results: list[RunResult | None], out: Output) -> None:
    ...
    if latencies:
        out.info(
            f"Average latency: {sum(latencies, timedelta()).total_seconds() / len(latencies) * 1000:.2f} ms"
        )
        ...
        out.info(
            f"90th percentile latency: {sorted(latencies)[int(len(latencies) * 0.9)].total_seconds() * 1000:.2f} ms"
        )
    ...
    console_histogram(
        [latency.total_seconds() * 1000 for latency in latencies], out, bins=10
    )
...
async def load(
    ...
    results: list[RunResult | None] = await run_load_test(
        scenario, activity, num, timeout
    )
