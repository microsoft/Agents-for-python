...
Example:
    from microsoft_agents.hosting.testing import scenario_registry, ExternalScenario
    ...
    def load_json(self, file_path: str) -> None:
        ...
        for name, body in agent_defs.items():
            ...
            if path_str.startswith(("http://", "https://")):
                ...
                if not script:
                    raise ValueError(
                        "A 'script' field is required for source scenarios"
                    )
...
def resolve_scenario(scenario_or_str: Scenario | str) -> Scenario:
    ...
    if isinstance(scenario_or_str, Scenario):
        ...
        if scenario_or_str.startswith("http://") or scenario_or_str.startswith(
            "https://"
        ):
