...
class SDKConfig:
    ...
    def __init__(self, env_path: Optional[str] = None, load_into_environment: bool = False):
        ...
        if load_into_environment:
            self._config = load_configuration_from_env(dotenv_values(env_path))  # Load .env file
        else:
            load_dotenv(env_path)  # Load .env file into environment variables
            self._config = load_configuration_from_env(os.environ)  # Load from environment variables
    ...
    def get_connection(self, connection_name: str = "SERVICE_CONNECTION") -> AgentAuthConfiguration:
