...
class _CLIConfig:
    ...
    def load_from_connection(self, connection_name: str = "SERVICE_CONNECTION", config: dict | None = None) -> None:
        """Load configuration from a connection dictionary."""

        config = config or dict(os.environ)

        config = {
            "app_id": os.environ.get(f"CONNECTIONS__{connection_name}__SETTINGS__CLIENTID", _UNSET),
            "app_secret": os.environ.get(f"CONNECTIONS__{connection_name}__SETTINGS__CLIENTSECRET", _UNSET),
            "tenant_id": os.environ.get(f"CONNECTIONS__{connection_name}__SETTINGS__TENANTID", _UNSET)
        }
