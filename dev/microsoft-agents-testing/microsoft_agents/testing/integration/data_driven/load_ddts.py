...
def load_ddts(path: str | Path | None = None, recursive: bool = False) -> list[DataDrivenTest]:
    ...
    for file_path, data in tests.items():
        ...
        if "parent" in data and isinstance(data["parent"], str):
            ...
            if not parent_data:
                raise ValueError(f"Parent test flow '{data['parent']}' not found for test flow '{file_path}'")
