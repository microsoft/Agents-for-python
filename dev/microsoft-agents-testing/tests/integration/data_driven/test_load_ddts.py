...
"""
    ...
    def test_load_with_parent_reference(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert child_test._input_defaults == {"activity": {"type": "message", "locale": "en-US"}}
    ...
    def test_load_resolves_parent_to_absolute_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create parent file
            parent_data = {"name": "parent", "defaults": {"input": {"activity": {"type": "message"}}}}
            parent_file = Path(temp_dir) / "parent.json"
            with open(parent_file, "w", encoding="utf-8") as f:
                json.dump(parent_data, f)
    ...
    def test_load_handles_mixed_json_and_yaml_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create JSON parent
            parent_data = {"name": "json_parent", "defaults": {"input": {"activity": {"type": "message"}}}}
            parent_file = Path(temp_dir) / "parent.json"
            with open(parent_file, "w", encoding="utf-8") as f:
                json.dump(parent_data, f)
