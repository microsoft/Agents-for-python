...
@pytest.fixture
def mock_agent_client(mock_transcript):
    """Create a mock AgentClient."""
    agent_client = MagicMock(spec=AgentClient)
    agent_client.transcript = mock_transcript
    agent_client.send = AsyncMock(return_value=[])
    agent_client.send_expect_replies = AsyncMock(return_value=[])
    agent_client.get_new = MagicMock(return_value=[])
    return agent_client
...
class TestConversationClientSay:
    ...
    @pytest.mark.asyncio
    async def test_say_without_expect_replies(self, mock_agent_client, sample_activities):
        """Test say() when expect_replies is False (default)."""
        mock_agent_client.send.return_value = sample_activities
        client = ConversationClient(mock_agent_client, expect_replies=False)
        
        result = await client.say("Hello")
        
        mock_agent_client.send.assert_called_once_with("Hello", wait=None, timeout=None)
        assert result == sample_activities

    @pytest.mark.asyncio
    async def test_say_with_expect_replies(self, mock_agent_client, sample_activities):
        """Test say() when expect_replies is True."""
        mock_agent_client.send_expect_replies.return_value = sample_activities
        client = ConversationClient(mock_agent_client, expect_replies=True)
        
        result = await client.say("Hello")
        
        mock_agent_client.send_expect_replies.assert_called_once_with("Hello", timeout=None)
        assert result == sample_activities

    @pytest.mark.asyncio
    async def test_say_with_timeout(self, mock_agent_client, sample_activities):
        ...
        mock_agent_client.send.assert_called_once_with("Hello", wait=None, timeout=5.0)

    @pytest.mark.asyncio
    async def test_say_with_expect_replies_and_timeout(self, mock_agent_client, sample_activities):
        ...
        mock_agent_client.send_expect_replies.assert_called_once_with("Hello", timeout=3.0)

    @pytest.mark.asyncio
    async def test_say_with_wait_parameter(self, mock_agent_client, sample_activities):
        ...
        mock_agent_client.send.assert_called_once_with("Hello", wait=2.0, timeout=None)

    @pytest.mark.asyncio
    async def test_say_returns_empty_list_when_no_responses(self, mock_agent_client):
        """Test say() returns empty list when no responses."""
        mock_agent_client.send.return_value = []
        client = ConversationClient(mock_agent_client)
...
class TestConversationClientWaitFor:
    ...
    @pytest.mark.asyncio
    async def test_wait_for_returns_immediately_when_match_found(self, mock_agent_client, sample_activities):
        """Test wait_for() returns immediately when matching activities are found."""
        mock_agent_client.get_new.return_value = sample_activities
        client = ConversationClient(mock_agent_client, timeout=5.0)
    ...
    @pytest.mark.asyncio
    async def test_wait_for_with_dict_filter(self, mock_agent_client, sample_activities):
    ...
    @pytest.mark.asyncio
    async def test_wait_for_with_callable_filter(self, mock_agent_client, sample_activities):
    ...
    @pytest.mark.asyncio
    async def test_wait_for_with_string_filter(self, mock_agent_client, sample_activities):
        """Test wait_for() with a string filter (text contains)."""
        mock_agent_client.get_new.return_value = sample_activities
        client = ConversationClient(mock_agent_client, timeout=5.0)
        
        result = await client.wait_for("Hello")
        
        assert result == sample_activities

    @pytest.mark.asyncio
    async def test_wait_for_polls_until_match(self, mock_agent_client, sample_activities):
        """Test wait_for() polls until a match is found."""
        # First two calls return empty, third returns activities
        mock_agent_client.get_new.side_effect = [[], [], sample_activities]
        client = ConversationClient(mock_agent_client, timeout=5.0)
        
        result = await client.wait_for(type=ActivityTypes.message)
        
        assert result == sample_activities
        assert mock_agent_client.get_new.call_count == 3

    @pytest.mark.asyncio
    async def test_wait_for_accumulates_activities(self, mock_agent_client, sample_activities):
        """Test wait_for() accumulates all activities while polling."""
        first_activity = [Activity(type=ActivityTypes.typing)]
        # First call returns typing, second returns messages
        mock_agent_client.get_new.side_effect = [first_activity, sample_activities]
        client = ConversationClient(mock_agent_client, timeout=5.0)
        
        result = await client.wait_for(type=ActivityTypes.message)
        
        # Should include all activities collected
        assert len(result) == 3
        assert result[0].type == ActivityTypes.typing

    @pytest.mark.asyncio
    async def test_wait_for_timeout_raises_timeout_error(self, mock_agent_client):
        """Test wait_for() raises TimeoutError when timeout is exceeded."""
        mock_agent_client.get_new.return_value = []  # Never matches
    ...
    @pytest.mark.asyncio
    async def test_wait_for_with_no_filter(self, mock_agent_client, sample_activities):
...
class TestConversationClientExpect:
    ...
    @pytest.mark.asyncio
    async def test_expect_succeeds_when_match_found(self, mock_agent_client, sample_activities):
        """Test expect() succeeds when matching activities are found."""
        mock_agent_client.get_new.return_value = sample_activities
        client = ConversationClient(mock_agent_client, timeout=5.0)
        
        # Should not raise - expect() doesn't return a value
        await client.expect(type=ActivityTypes.message)

    @pytest.mark.asyncio
    async def test_expect_raises_assertion_error_on_timeout(self, mock_agent_client):
    ...
    @pytest.mark.asyncio
    async def test_expect_with_dict_filter(self, mock_agent_client, sample_activities):
    ...
    @pytest.mark.asyncio
    async def test_expect_with_callable_filter(self, mock_agent_client, sample_activities):
    ...
    @pytest.mark.asyncio
    async def test_expect_with_kwargs(self, mock_agent_client, sample_activities):
...
class TestConversationClientIntegration:
    ...
    @pytest.mark.asyncio
    async def test_conversation_flow(self, mock_agent_client, sample_activities):
    ...
    @pytest.mark.asyncio
    async def test_timeout_can_be_changed_mid_conversation(self, mock_agent_client, sample_activities):
        ...
        mock_agent_client.send.assert_called_with("First message", wait=None, timeout=1.0)
        
        # Change timeout
        client.timeout = 10.0
        
        await client.say("Second message")
        mock_agent_client.send.assert_called_with("Second message", wait=None, timeout=10.0)

    @pytest.mark.asyncio
    async def test_expect_replies_mode(self, mock_agent_client, sample_activities):
        ...
        mock_agent_client.send_expect_replies.assert_called_once()
        mock_agent_client.send.assert_not_called()
        assert responses == sample_activities

    @pytest.mark.asyncio
    async def test_transcript_access(self, mock_agent_client, mock_transcript):
        """Test that transcript is accessible throughout conversation."""
        client = ConversationClient(mock_agent_client, timeout=5.0)
        
        # Transcript should be the same as the agent client's transcript
        assert client.transcript is mock_transcript